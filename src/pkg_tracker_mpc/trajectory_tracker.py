...
class TrajectoryTracker:
    ...
    def get_dyn_constraints(self, dyn_obstacle_list=None):
        """Get the parameters for dynamic obstacle constraints from a list of dynamic obstacles.

        Args:
            dyn_obstacle_list: Each element contains info about one dynamic obstacle. Defaults to None.
        """
        params_per_dyn_obs = self.config.ndynobs
        dyn_constraints = [0.0] * self.config.Ndynobs * params_per_dyn_obs
        if dyn_obstacle_list is not None:
            for i, dyn_obstacle in enumerate(dyn_obstacle_list):
                # dyn_constraints[i*params_per_dyn_obs:(i+1)*params_per_dyn_obs] = list(itertools.chain(*dyn_obstacle))
                dyn_constraints[i*params_per_dyn_obs:(i+1)*params_per_dyn_obs] = dyn_obstacle
    ...
    def set_ref_states(self, ref_states: np.ndarray, ref_speed:Optional[float]=None):
        ...
        Notes:
            This method will overwrite the base speed.
        """
        self.ref_states = ref_states[:self.N_hor, :]
    ...
    def run_step(self, static_obstacles: list[list[PathNode]], dyn_obstacle_list:Optional[list]=None, other_robot_states:Optional[list]=None, 
        ...
        Args
            ...
            dyn_obstacle_list: A list of dynamic obstacles. Defaults to None.
        ...
        dyn_constraints = self.get_dyn_constraints(dyn_obstacle_list)
