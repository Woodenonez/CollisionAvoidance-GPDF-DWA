...
class PanocBuilder:
    ...
    def _load_variables(self):
        ...
        self._o_d = ca.SX.sym('od', self._cfg.Ndynobs*self._cfg.ndynobs)  # 10. Dynamic obstacles
    ...
    def step_cost(self, step_in_horizon: int, action: ca.SX, last_state: ca.SX,
        ...
        r_dyn = dynamic_obstacles[2::self._cfg.ndynobs*(self.N_hor+1)]
        circle_param = [x_dyn, y_dyn, r_dyn+self._spec.vehicle_width/2+self._spec.vehicle_margin+self._spec.social_margin]
        cts.cost_dynobs += mc.cost_inside_circles(state.T, circle_param, weight=self._large_weight)

        inside_dyn_obstacle = mh.inside_circles(state, [x_dyn, y_dyn, r_dyn+self._spec.vehicle_width/2+self._spec.vehicle_margin])
