...
def inside_circles(point: cs.SX, circle_param: list[cs.SX]) -> cs.SX:
    """Check if a point is inside a set of circles.

    Args:
        point: The (n*1)-dim target point.
        circle_param: Shape (3*m) with m circles. Each circle is defined by (cx, cy, r).

    Returns:
        is_inside: The (1*m)-dim indicator vector. If inside, return positive value, else return negative value.
    """
    x, y = point[0], point[1]
    cx, cy, r = circle_param[0], circle_param[1], circle_param[2]
    is_inside = r**2 - ((x-cx)**2 + (y-cy)**2)
    return is_inside
