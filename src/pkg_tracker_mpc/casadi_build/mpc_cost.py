...
def cost_inside_circles(state: ca.SX, circle_param: list[ca.SX], weight:Union[ca.SX, float]=1.0) -> ca.SX:
    """Cost (weighted squared) for being inside a set of circles defined by `(cx, cy, r)`.
    ...
    """
    indicator = inside_circles(state, circle_param) # indicator<0, if outside circle
    indicator = weight * ca.fmax(0.0, indicator)**2
