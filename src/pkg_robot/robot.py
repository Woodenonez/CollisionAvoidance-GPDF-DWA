...
class RobotManager():
    ...
    @_check_id
    def get_other_robot_states(self, 
                               ego_robot_id, 
                               n_state: int, 
                               n_horizon: int, 
                               n_others: int, 
        ...
        if not sep:
            ...
            other_robot_state_dict = {}
            for id_ in list(self._robot_dict):
                if id_ != ego_robot_id:
                    current_state = self.get_robot_state(id_)
                    pred_states = self.get_pred_states(id_)
                    other_robot_state_dict[id_] = list(current_state)
                    if pred_states is not None:
                        other_robot_state_dict[id_] += list(pred_states.reshape(-1))
            return other_robot_state_dict
