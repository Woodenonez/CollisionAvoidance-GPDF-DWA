...
class TrajectoryTracker:
    ...
    def calc_traj_cost(self, trajectory: np.ndarray, action: np.ndarray, ref_path: np.ndarray, goal_state: np.ndarray, 
        ...
        if static_obstacles is not None:
            cost_static_obs = self.calc_cost_static_obstacles(trajectory, static_obstacles, thre=safe_thre) # type: ignore
    ...
    def run_step_gpdf(self, 
        ...
        # print(f"[{self.__class__.__name__}-{self.robot_id}] GPDF time: {timer() - start_time:.5f}s")
